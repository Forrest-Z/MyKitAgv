  ...
  def serialize(self, buff):
    ...
    try:
      ...
      for val1 in self.monitoring_cases:
        buff.write(_get_struct_i().pack(val1.monitoring_case_number))
        length = len(val1.fields)
        buff.write(_struct_I.pack(length))
        pattern = '<%si'%length
        buff.write(struct.pack(pattern, *val1.fields))
        length = len(val1.fields_valid)
        buff.write(_struct_I.pack(length))
        pattern = '<%sB'%length
        buff.write(struct.pack(pattern, *val1.fields_valid))
    except struct.error as se: self._check_types(struct.error("%s: '%s' when writing '%s'" % (type(se), str(se), str(locals().get('_x', self)))))
    except TypeError as te: self._check_types(ValueError("%s: '%s' when writing '%s'" % (type(te), str(te), str(locals().get('_x', self)))))
  ...
  def deserialize(self, str):
    ...
    try:
      ...
      for i in range(0, length):
        ...
        val1.fields = struct.unpack(pattern, str[start:end])
        start = end
        end += 4
        (length,) = _struct_I.unpack(str[start:end])
        pattern = '<%sB'%length
        ...
        val1.fields_valid = struct.unpack(pattern, str[start:end])
        val1.fields_valid = map(bool, val1.fields_valid)
