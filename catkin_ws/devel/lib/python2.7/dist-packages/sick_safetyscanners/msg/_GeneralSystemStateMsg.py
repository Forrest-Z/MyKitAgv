...
"""
  ...
  def serialize(self, buff):
    ...
    try:
      ...
      pattern = '<%sB'%length
      buff.write(struct.pack(pattern, *self.safe_cut_off_path))
      length = len(self.non_safe_cut_off_path)
      buff.write(_struct_I.pack(length))
      pattern = '<%sB'%length
      buff.write(struct.pack(pattern, *self.non_safe_cut_off_path))
      length = len(self.reset_required_cut_off_path)
      buff.write(_struct_I.pack(length))
      pattern = '<%sB'%length
      buff.write(struct.pack(pattern, *self.reset_required_cut_off_path))
      _x = self
  ...
  def deserialize(self, str):
    ...
    try:
      ...
      self.safe_cut_off_path = struct.unpack(pattern, str[start:end])
      self.safe_cut_off_path = map(bool, self.safe_cut_off_path)
      start = end
      end += 4
      (length,) = _struct_I.unpack(str[start:end])
      pattern = '<%sB'%length
      ...
      self.non_safe_cut_off_path = struct.unpack(pattern, str[start:end])
      self.non_safe_cut_off_path = map(bool, self.non_safe_cut_off_path)
      start = end
      end += 4
      (length,) = _struct_I.unpack(str[start:end])
      pattern = '<%sB'%length
      ...
      self.reset_required_cut_off_path = struct.unpack(pattern, str[start:end])
      self.reset_required_cut_off_path = map(bool, self.reset_required_cut_off_path)
      _x = self
      start = end
      end += 6
